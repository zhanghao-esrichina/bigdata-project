...
def create_fishnet(in_raster_path, out_fc_path):
    ...
    # Run fishnet tool
    logger.info(
        "Running fishnet creator: {} with PID {}".format(
...
def execute_task(in_extentDict):
    ...
    logger.info(
        "Running local math task: {} with PID {}".format(
            current_process().name,
            os.getpid()))
    # run the local task
    ...
    time2 = time.clock()
    logger.info(
        "{} with PID {} finished in {}".format(
            current_process().name, os.getpid(), str(
                time2 - time1)))
...
if __name__ == '__main__':
    ...
    # get extents of individual chunk features in the fishnet, add the extents
    # to a dictonary
    extDict = {}
    count = 1
    ...
    arcpy.AddRastersToMosaicDataset_management(
        r"mosaic_out.gdb\localFn_Mosaic", "Raster Dataset", in_path)
    # end the clock
    time_end = time.clock()
    logger.info("Time taken in main in seconds(s) is : {}".format(
